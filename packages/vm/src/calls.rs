...
use crate::imports::{ reset_db_read,get_all_time,get_db_read,get_detail};
    ...
    Q: Querier + 'static,
{
    ...
    let( read_region,store_get,process_gas,write_ro_contract)=get_db_read();
    let (read_region_get_region,read_region_handle,write_read_region,write_call_function,gaas_cal,gas_set)=get_detail();

    println!("all_db_cnt {:?}  all_read_time {:?}",db_cnt,all_read_ts);
    println!("do_db_read -> read_region {:?} store_get {:?} process_gas {:?} write_to_contract {:?}",read_region,store_get,process_gas,write_ro_contract);
    println!("read_region -> get_region {:?} handle region {:?}",read_region_get_region,read_region_handle);
    println!("process_gas -> cal {:?} set {:?}",gaas_cal,gas_set);
    println!("write_to_contract -> call_function {:?} write_region {:?}",write_read_region,write_call_function);
    Ok(data)
}
